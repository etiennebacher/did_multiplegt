...
did_multiplegt <- function(
  ...
  df_preprocess <- did_multiplegt_preprocess(
    df,
    Y,
    G,
    T,
    ...
    if (covariance) {
      ...
      for (i in 1:placebo) {
        ...
        for (j in (i + 1):placebo) {
          ...
          name <-
            paste("cov_placebo", toString(i), toString(j), sep = "_")
          ret[name] <- r
        }
      }
      # dynamic
      for (i in 0:dynamic) {
        ...
        for (j in (i + 1):dynamic) {
          ...
          name <-
            paste("cov_effect", toString(i), toString(j), sep = "_")
          ret[name] <- r
        }
      }
      ...
      if (!is.null(average_effect)) {
        ...
        effect_average <-
          sum(unlist(eret[names[start:end]]) * weight)
        N_effect_vec <- unlist(eret[mapply(function(x) {
          paste("N", x, sep = "_")
        }, names[start:end])])
        N_effect_average <- sum(N_effect_vec)
        var_effect_vec <- unlist(ret[mapply(function(x) {
          paste("se", x, sep = "_")
        }, names[start:end])])
        ...
        for (i in 1:dynamic) {
          ...
          for (j in (i + 1):dynamic) {
            ...
            var_effect_average <-
              var_effect_average + weight[i + 1] * weight[j + 1] * 2 * unlist(ret[name])
          }
        }
...
printf <- function(...)
  print(sprintf(...))
fn_ctrl_rename <- function(x) {
  paste("ctrl", x, sep = "_")
}
fn_diff_rename <- function(x) {
  paste("diff", x, sep = "_")
}
fn_diff_i_rename <- function(x, i) {
  paste("diff", toString(i), "_", x, sep = "")
}
fn_lag_rename <- function(x, k) {
  paste("L", toString(k), "_", x, sep = "")
}
fn_lead_rename <- function(x, k) {
  paste("F", toString(k), "_", x, sep = "")
}

get_controls_rename <- function(controls) {
  unlist(lapply(controls, fn_ctrl_rename))
}
get_diff_controls_rename <- function(controls) {
  unlist(lapply(controls, function(x) {
    fn_diff_rename(fn_ctrl_rename(x))
  }))
}
...
generate_switchers_names <- function(dynamic = 0, placebo = 0) {
  ...
  if (dynamic > 0) {
    for (i in 1:dynamic) {
      nm_N_switchers_dynamic <-
        paste("N", "switchers", "effect", toString(i), sep = "_")
...
did_multiplegt_transform <- function(df, controls, trends_nonparam) {
  ...
  df <- as_tibble(df)
    ...
    if ("Vtrends" %in% colnames(df)) {
      ignore_columns <- c(ignore_columns, "Vtrends")
    }

    df <- df %>%
      mutate(
        # mark if a row has a missing value anywhere but in Y
        tag = rowSums(is.na(pick(-Y)))
      ) %>%

      # if a row has a missing value in any col, add NA in all other columns
      mutate(
        across(
          .cols = -any_of(ignore_columns),
          .fns = function(x) ifelse(tag > 0, NA, x)
        )
      ) %>%
      select(-tag)

    # canonicalize treatment to start from 0 and increase consecutively
    cat_treatment <- if ("Drecat" %in% colnames(df)) {
      "Drecat"
    } else {
      "D"
    }
    ...
    tdf <- df %>%
      ...
      select(!!var_cat_treatment, Dgroup)
    ...
    if (trends_nonparam) {
      tdf <- df %>%
        group_by(Vtrends, Tfactor) %>%
        summarize(VtrendsX = cur_group_id()) %>%
        select(Vtrends, Tfactor, VtrendsX)
      tdf$VtrendsX <- factor(tdf$VtrendsX)

      df <- left_join(df, tdf, by = c("Vtrends", "Tfactor")) %>%
        mutate(Vtrends = VtrendsX) %>%
        select(-VtrendsX)
    }
...
placebo_transform_level <- function(df, counter_placebo, controls) {
  nm_diff_controls <- unlist(lapply(controls, function(x) {
    fn_diff_rename(fn_ctrl_rename(x))
  }))
  nm_lag_diff_D <- fn_lag_rename("diff_D", counter_placebo)
  fn_lag_rename_i <- function(x) {
...
dynamic_transform_level <- function(df, counter_dynamic, controls) {
  ...
  nm_diff_i_controls <- unlist(lapply(nm_ctrls, function(x) {
    fn_diff_i_rename(x, counter_dynamic)
  }))
  nm_diff_i_Y <- fn_diff_i_rename("Y", counter_dynamic)

  fn_lead_rename_i <- function(x) {
  ...
  df_l <- df %>%
    ...
    mutate_at(
      .vars = vars(c("Y", nm_ctrls)),
      .funs = function(x) {
        x - lag(x, counter_dynamic + 1)
      }
    ) %>%
    rename_at(
      .vars = vars(c("Y", nm_ctrls)),
    ...
    mutate_at(
      .vars = vars(lead_names),
      .funs = function(x) {
        lead(x, counter_dynamic)
      }
    ) %>%
...
placebo_update_cond <-
  function(df,
    ...
    df %>% mutate(
      placebo_cond = ifelse(
        !is.na(!!var_lag_diff_D) &
          abs(!!var_lag_diff_D) <= threshold_stable_treatment,
        placebo_cond,
        FALSE
      )
    )
...
prepare_placebo_once <-
  function(df,
    ...
    nm_lag_diff_D <- fn_lag_rename("diff_D", counter_placebo)
    nm_lag_diff_controls <-
      unlist(lapply(nm_diff_controls, function(x) {
        fn_lag_rename(x, counter_placebo)
      }))
    ...
    df <- df %>%
      ...
      select_at(.vars = vars(-c(
        "diff_Y", nm_diff_controls, nm_lag_diff_D
      ))) %>%
...
dynamic_update_cond <-
  function(df,
    ...
    df %>% mutate(
      dynamic_cond = ifelse(
        !is.na(!!var_lead_diff_D) &
          abs(!!var_lead_diff_D) <= threshold_stable_treatment,
        dynamic_cond,
        FALSE
      )
    )
...
prepare_dynamic_once <-
  function(df,
    ...
    nm_ctrls <- unlist(lapply(controls, fn_ctrl_rename))
    nm_diff_controls <- unlist(lapply(nm_ctrls, function(x) {
      fn_diff_rename(x)
    }))
    ...
    nm_lead_diff_i_controls <-
      unlist(lapply(nm_diff_i_controls, function(x) {
        fn_lead_rename(x, counter_dynamic)
      }))

    prefix <-
      paste("F",
            toString(counter_dynamic),
            "_",
            "diff",
            toString(counter_dynamic),
            "_",
    ...
    df <- df %>%
      ...
      rename_at(.vars = vars(c(
        nm_lead_diff_i_Y, nm_lead_diff_i_controls
      )), .funs = list( ~ str_replace(., prefix, "diff_"))) %>%
...
residuals_control_once <-
  function(df,
    ...
    df_stable <- df_d %>%
      filter(abs(diff_D) <= threshold_stable_treatment) %>%
      filter(!is.na(diff_Y))
    ...
    if (n_stable <= length(diff_ctrl_names)) {
      return(
        df_d %>% mutate(residuals = diff_Y) %>% select(T, G, residuals)
      )
    }
    ...
    if (do_fixed_effect & ("Vtrends" %in% colnames(df_stable))) {
      ...
      df_stable.lm <-
        feols(as.formula(formula),
              data = df_stable,
              weights = ~ counter)
      df_trends <-
        df_d %>% filter(Vtrends %in% levels(df_stable$Vtrends))
      df_trends$pred_trends <- predict(df_stable.lm, df_trends)
      df_trends <-
        df_trends %>% select(T, G, pred_trends)
    }

    # normal controls
    formula <- paste(diff_ctrl_names, collapse = " + ")
    if (do_count_tfactor)
      formula <- paste(formula, " + Tfactor")
    formula <- paste("diff_Y ~ ", formula, sep = "")
    ...
    df_controls$pred_controls <-
      predict(df_stable.lm, df_controls, allow.new.levels = TRUE)
    df_controls <-
      df_controls %>% select(T, G, pred_controls)

    if (is.null(df_trends)) {
      df_d %>%
        left_join(df_controls, by = c("T", "G")) %>%
        mutate(residuals = case_when(
      ...
      df_d %>%
        ...
        mutate(
          residuals = case_when(
            !is.na(pred_trends) ~ diff_Y - pred_trends,
            !is.na(pred_controls) ~ diff_Y - pred_controls,
            TRUE ~ diff_Y
          )
        ) %>%
...
residuals_control <-
  function(df,
    ...
    diff_ctrl_names <- unlist(lapply(controls, function(x) {
      fn_diff_rename(fn_ctrl_rename(x))
    }))
    ...
    for (d in d_min:d_max) {
      tmp_ret <- residuals_control_once(df,
                                        d,
                                        threshold_stable_treatment,
                                        diff_ctrl_names,
                                        trends_nonparam,
...
did_multiplegt_once <- function(df, d, threshold_stable_treatment) {
  ...
  df_increase <-
    tmp_cnt_df %>% filter(diff_D > threshold_stable_treatment)
  n_increase <- as.numeric(nrow(df_increase))
  cnt_increase <- as.numeric(sum(df_increase$counter, na.rm = TRUE))

  df_stable <-
    tmp_cnt_df %>% filter(abs(diff_D) <= threshold_stable_treatment)
  ...
  if (n_increase * n_stable > 0) {
    ...
    df_plus$treatment <-
      case_when(df_plus$diff_D > threshold_stable_treatment ~ 1, TRUE ~ 0)

    counter <- df_plus$counter

    df_plus.lm <-
      lm(diff_Y ~ treatment, data = df_plus, weights = counter)
    ret$effect <-
      ret$effect + df_plus.lm$coefficients["treatment"] * cnt_increase

    df_plus.lm <-
      lm(diff_D ~ treatment, data = df_plus, weights = counter)
    ret$denom <-
      ret$denom + df_plus.lm$coefficients["treatment"] * cnt_increase
  }
  ...
  if (n_decrease * n_stable > 0) {
    ...
    df_minus$treatment <-
      case_when(df_minus$diff_D < -threshold_stable_treatment ~ 1, TRUE ~ 0)

    counter <- df_minus$counter

    df_minus.lm <-
      lm(diff_Y ~ treatment, data = df_minus, weights = counter)
    ret$effect <-
      ret$effect - df_minus.lm$coefficients["treatment"] * cnt_decrease

    df_minus.lm <-
      lm(diff_D ~ treatment, data = df_minus, weights = counter)
    ret$denom <-
      ret$denom - df_minus.lm$coefficients["treatment"] * cnt_decrease
  }
...
did_multiplegt_core <-
  function(df,
    ...
    for (t in (counter_placebo + 2):(t_max - counter_dynamic)) {
      df_d <- df %>%
        filter(T == t) %>%
        filter(!is.na(diff_D) & !is.na(diff_Y))
      if (nrow(df_d) > 0) {
    ...
    list(
      effect = effect / denom,
      N_effect = total,
      N_switchers_effect = switchers
    )
  }

did_multiplegt_estim <-
  function(df,
           controls,
           placebo,
           dynamic,
    ...
    if (dynamic > 0) {
      ...
      for (i in 1:dynamic) {
        ...
        df_dynamic_preprocessed <-
          dynamic_update_cond(df_dynamic_preprocessed,
                              i,
                              controls,
                              threshold_stable_treatment)
        df_dynamic_lead <- prepare_dynamic_once(df_dynamic_preprocessed,
                                                i,
                                                controls,
                                                threshold_stable_treatment)
        ...
        ret[nm_N_switchers_dynamic] <-
          ret_dynamic_lag$N_switchers_effect
      }
    }
    ...
    if (placebo > 0) {
      ...
      for (i in 1:placebo) {
        ...
        df_placebo_preprocessed <-
          placebo_update_cond(df_placebo_preprocessed,
                              i,
                              controls,
                              threshold_stable_treatment)
        df_placebo_lag <- prepare_placebo_once(df_placebo_preprocessed,
                                               i,
                                               controls,
                                               threshold_stable_treatment)
...
did_multiplegt_bootstrap <-
  function(df,
    ...
    if (parallel) {
      ...
      matrix(
        unlist(mclapply(1:brep, callback, mc.cores = numCores)),
        nrow = brep,
        ncol = K,
        byrow = TRUE
      )
    } else {
      matrix(unlist(lapply(1:brep, callback)),
             nrow = brep,
             ncol = K,
             byrow = TRUE)
    }
