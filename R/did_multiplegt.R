...
did_multiplegt <- function(df, Y, G, T, D, controls = c(),
  ...
  eret <- did_multiplegt_estim(
    df_preprocess, controls, placebo, dynamic, threshold_stable_treatment,
    trends_nonparam, trends_lin
  )
...
did_multiplegt_transform <- function(df, controls, trends_nonparam) {
  ...
  df <- df %>%
    ...
    mutate_at(.vars = vars(-ignore_columns), .funs = list(~ ifelse(.data$tag, NA, .))) %>%
    select(-.data$tag)
  ...
  ddf <- df %>%
    ...
    group_by(.data$G) %>%
    mutate_at(.vars = columns_to_diff, .funs = function(x) {
      x - lag(x)
    }) %>%
  ...
  if (trends_nonparam) {
    ...
    df <- left_join(df, tdf, by = c("Vtrends", "Tfactor")) %>%
      mutate(Vtrends = .data$VtrendsX) %>%
      select(-.data$VtrendsX)
  }
...
placebo_transform_level <- function(df, counter_placebo, controls) {
  ...
  df_l <- df %>%
    ...
    mutate_at(.vars = vars(lag_names), .funs = function(x) {
...
dynamic_transform_level <- function(df, counter_dynamic, controls) {
  ...
  df_l <- df %>%
    ...
    mutate_at(.vars = vars(c("Y", nm_ctrls)), .funs = function(x) {
      x - lag(x, counter_dynamic + 1)
    }) %>%
    rename_at(.vars = vars(c("Y", nm_ctrls)), .funs = function(x) {
      fn_diff_i_rename(x, counter_dynamic)
    }) %>%
    mutate_at(.vars = vars(lead_names), .funs = function(x) {
...
residuals_control_once <- function(df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin) {
  ...
  df_stable <- df_d %>%
    filter(abs(.data$diff_D) <= threshold_stable_treatment) %>%
    filter(!is.na(.data$diff_Y))
  ...
  if (is.null(df_trends)) {
    ...
    df_d %>%
      ...
      left_join(df_controls, by = c("T", "G")) %>%
      mutate(residuals = case_when(
        !is.na(.data$pred_trends) ~ .data$diff_Y - .data$pred_trends,
        !is.na(.data$pred_controls) ~ .data$diff_Y - .data$pred_controls,
        TRUE ~ .data$diff_Y
      )) %>%
      select(.data$T, .data$G, .data$residuals)
...
did_multiplegt_core <- function(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin,
  ...
  for (t in (counter_placebo + 2):(t_max - counter_dynamic)) {
    df_d <- df %>%
      filter(.data$T == t) %>%
      filter(!is.na(.data$diff_D) & !is.na(.data$diff_Y))
    if (nrow(df_d) > 0) {
...
did_multiplegt_estim <- function(df, controls, placebo, dynamic, threshold_stable_treatment, trends_nonparam, trends_lin) {
  ...
  if (dynamic > 0) {
    ...
    for (i in 1:dynamic) {
      ...
      df_dynamic_lead <- prepare_dynamic_once(
        df_dynamic_preprocessed, i, controls,
  ...
  if (placebo > 0) {
    ...
    for (i in 1:placebo) {
      ...
      df_placebo_lag <- prepare_placebo_once(
        df_placebo_preprocessed, i, controls,
        threshold_stable_treatment
      )
...
plot_bootstrap_result <- function(placebo, dynamic, X, LB, UB) {
  ...
  plotCI(points, X,
    ui = UB, li = LB, col = "navyblue", scol = "firebrick4", pch = 19, cex = 0.5,
